...
function hamilton_smoother(y::AbstractMatrix,
    ...
    Nt = size(y, 2)
    hamilton_smoother(AbstractRange{Int}[1:Nt], y, Matrix{S}[T], Matrix{S}[R], Vector{S}[C],
        Matrix{S}[Q], Matrix{S}[Z], Vector{S}[D], Matrix{S}[E], s_0, P_0; Nt0 = Nt0)
end

function hamilton_smoother(regime_indices::Vector{AbstractRange{Int}}, y::AbstractMatrix,
