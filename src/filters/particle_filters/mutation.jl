...
Called by tempered particle filter.
...
Called by bootstrap particle filter.
...
the new `s_t`, `ϵ_t`, and the number of acceptances `accept`. The matrices det_HH and inv_HH
are 'scaled' in the case of the tempered particle filter, but are exact when called by
...
function mh_steps(Φ::Function, Ψ::Function, dist_ϵ::MvNormal, y_t::Vector{Float64},
    ...
    post_1::Float64 = fast_mvnormal_pdf(y_t - Ψ(s_t), scaled_det_HH, scaled_inv_HH)
    ...
    for j = 1:n_mh_steps
        ...
        # Calculate posterior
        post_new_1::Float64 = fast_mvnormal_pdf(y_t - Ψ(s_new), scaled_det_HH, scaled_inv_HH)
