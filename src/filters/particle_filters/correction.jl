...
Used by tempered particle filter.
...
Used by bootstrap particle filter.
...
"""
function weight_kernel!(coeff_terms::V, log_e_terms::V, Ψ::Function,
...
"""
```
correction!(inc_weights, norm_weights, coeff_terms, log_e_terms, n_obs)
```
...
Compute (and modify in-place) incremental weights w̃ₜʲ and normalized weights W̃ₜʲ:
        ...
        w̃ₜʲ = pₙ(yₜ|sₜʲ'ⁿ⁻¹) / pₙ₋₁(yₜ|sₜʲ'ⁿ⁻¹)
            = exp{-1/2 [yₜ-Ψ(sₜʲ'ⁿ⁻¹)]' Σᵤ⁻¹ [yₜ-Ψ(sₜʲ'ⁿ⁻¹)]}
