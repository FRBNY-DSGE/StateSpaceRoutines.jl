...
states[:hamilton], shocks[:hamilton] = hamilton_smoother(data, TTT, RRR, z0, pred, vpred, filt, vfilt)
states[:koopman], shocks[:koopman] = koopman_smoother(data, TTT, RRR, CCC, QQ, ZZ, DD, z0, P0, pred, vpred)
states[:carter_kohn], shocks[:carter_kohn] = carter_kohn_smoother(data, TTT, RRR, z0, pred, vpred, filt, vfilt; draw_states = false)
